...
targetTrack.notes.forEach((v) => {
    ...
    for (let i = 0; i < voices * 2; i += 2) {
        ...
        voiceCounters[i]++;
        lfoRepr_1.applyNote(targetTrack, data.settings.modulations[i].line_mapping, data.settings.modulations[i + 1].line_mapping, noteStartPercent, noteEndPercent, v.name, voiceNotes[i]);
        voiceNotes[i] = v.name;
        break;
    }
});
